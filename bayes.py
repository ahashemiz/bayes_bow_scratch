...
def vocab(neg:Sequence[Sequence], pos:Sequence[Sequence]) -> dict:
    ...
    all_words_set = set()
    for neg_doc in neg:
        all_words_set = all_words_set.union(set(neg_doc))
    for pos_doc in pos:
        all_words_set = all_words_set.union(set(pos_doc))
    all_words_list = list(all_words_set)
    all_words_list.append('__unknown__')
    sorted_words = sorted(all_words_list)
...
def vectorize(V:dict, docwords:Sequence) -> np.ndarray:
    ...
    for word in docwords:
        idx = V[word]
        row_vector[idx] = row_vector[idx] + 1
    return row_vector
...
class NaiveBayes621:
    ...
    def fit(self, X:np.ndarray, y:np.ndarray) -> None:
        ...
        self.p_c = np.array([1-p_ones, p_ones])
        one_mask = np.where(y==1)[0]
        zero_mask = np.where(y==0)[0]
        X_zeros = X[zero_mask]
        X_ones = X[one_mask]
        wordcount_w_zeros = np.sum(X_zeros, axis=0)
        wordcount_w_ones = np.sum(X_ones, axis=0)
        P_w_zeros = (wordcount_w_zeros + 1) / (X_zeros.sum().sum() + V)
        P_w_ones = (wordcount_w_ones + 1) / (X_ones.sum().sum() + V)
        self.P_W_C = np.array([P_w_zeros, P_w_ones])
    ...
    def predict(self, X:np.ndarray) -> np.ndarray:
        ...
        """
        P_zero = np.log(self.p_c[0]) + np.dot(X, np.log(self.P_W_C[0]))
        P_one = np.log(self.p_c[1]) + np.dot(X, np.log(self.P_W_C[1]))
        y_preds = np.argmax(np.column_stack((P_zero, P_one)), axis=1)
        return y_preds
